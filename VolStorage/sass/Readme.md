...
folder are mapped to the application's root namespace, 'VolStorage'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "VolStorage"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'VolStorage' in the same way
as `"VolStorage/sass/src"`.
